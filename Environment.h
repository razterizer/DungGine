  ...
  public:
                          ...
                          Player& player)
    {
      ...
      for (const auto& room_pair : m_room_styles)
      {
        ...
        if (player.curr_room == room && player.is_inside_curr_room())
          player.on_terrain = get_terrain(player.pos);
      ...
      for (const auto& corr_pair : m_corridor_styles)
      {
        ...
        if (player.curr_corridor == corr && player.is_inside_curr_corridor())
          player.on_terrain = Terrain::Default;
